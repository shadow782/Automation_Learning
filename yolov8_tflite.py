...
class Yolov8TFLite:
    def __init__(self, tflite_model, input_image, confidence_thres, iou_thres, ext_delegate):
        """
    ...
    def main(self):
        ...
        interpreter = tflite.Interpreter(model_path=self.tflite_model, experimental_delegates=self.ext_delegate)
        ...
        pipeline = 'filesrc location=' + self.input_image + ' ! qtdemux ! h264parse ! vpudec ! videoconvert ! video/x-raw format=RGB ! appsink'

        #self.cap = cv2.VideoCapture(self.input_image)
        cap = cv2.VideoCapture(pipeline)

        videoWriter = cv2.VideoWriter('output.avi', cv2.VideoWriter_fourcc(*'H264'), 30, (1280, 720))
        while(cap.isOpened()):
...
if __name__ == "__main__":
    ...
    detection = Yolov8TFLite(args.model, args.img, args.conf_thres, args.iou_thres, args.ext_delegate)
